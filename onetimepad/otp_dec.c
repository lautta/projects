...
int main(int argc, char *argv[])
{
	...
	// generic value that holds the number of bytes received or sent
	int numBytes;
	...
	char cipherText[textSize], keyText[keySize], plainText[textSize];
	...
	getFile(textName, cipherText, textSize);
	getFile(keyName, keyText, keySize);

	// contact the server and create a socket
	socketFD = makeContact(portNumber);

	// perform the secret handshake, make sure dec only connects to dec servers
	...
	// send the contents of the ciphertext and key files
	if ((numBytes = send(socketFD, cipherText, sizeof(cipherText), 0)) < 0)
	{
		fprintf(stderr, "Error: could not send cipher text to server\n");
		close(socketFD);
		exit(1);
	}
	...
	// receive the deciphered text contents back using the MSG_WAITALL flag
	...
	if ((numBytes = recv(socketFD, plainText, sizeof(plainText), MSG_WAITALL)) < 0)
	{
		fprintf(stderr, "Error: could not receive plain text from server\n");
		close(socketFD);
		exit(1);
	}
...
void handShake(int socketFD, int portNumber)
{
	...
	// copy the passcode "DE" for decryption and send it to server
